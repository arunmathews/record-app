...
trait Tables {
  ...
  lazy val schema: profile.SchemaDescription = FlywaySchemaHistory.schema ++ Record.schema
   ...
   *  @param lastName Database column last_name SqlType(VARCHAR), Length(300,true)
   *  @param firstName Database column first_name SqlType(VARCHAR), Length(300,true)
   *  @param gender Database column gender SqlType(VARCHAR), Length(300,true)
   *  @param favoriteColor Database column favorite_color SqlType(VARCHAR), Length(300,true)
   *  @param birthDate Database column birth_date SqlType(DATE) */
  case class RecordRow(id: Int, lastName: String, firstName: String, gender: String, favoriteColor: String, birthDate: java.sql.Date)
  /** GetResult implicit for fetching RecordRow objects using plain SQL queries */
  implicit def GetResultRecordRow(implicit e0: GR[Int], e1: GR[String], e2: GR[java.sql.Date]): GR[RecordRow] = GR{
    prs => import prs._
    RecordRow.tupled((<<[Int], <<[String], <<[String], <<[String], <<[String], <<[java.sql.Date]))
  }
  ...
  class Record(_tableTag: Tag) extends profile.api.Table[RecordRow](_tableTag, "record") {
    def * = (id, lastName, firstName, gender, favoriteColor, birthDate) <> (RecordRow.tupled, RecordRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(lastName), Rep.Some(firstName), Rep.Some(gender), Rep.Some(favoriteColor), Rep.Some(birthDate))).shaped.<>({r=>import r._; _1.map(_=> RecordRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column id SqlType(INTEGER), AutoInc, PrimaryKey */
    val id: Rep[Int] = column[Int]("id", O.AutoInc, O.PrimaryKey)
    /** Database column last_name SqlType(VARCHAR), Length(300,true) */
    val lastName: Rep[String] = column[String]("last_name", O.Length(300,varying=true))
    /** Database column first_name SqlType(VARCHAR), Length(300,true) */
    val firstName: Rep[String] = column[String]("first_name", O.Length(300,varying=true))
    /** Database column gender SqlType(VARCHAR), Length(300,true) */
    val gender: Rep[String] = column[String]("gender", O.Length(300,varying=true))
    /** Database column favorite_color SqlType(VARCHAR), Length(300,true) */
    val favoriteColor: Rep[String] = column[String]("favorite_color", O.Length(300,varying=true))
    /** Database column birth_date SqlType(DATE) */
    val birthDate: Rep[java.sql.Date] = column[java.sql.Date]("birth_date")

    /** Index over (lastName) (database name record_idx1) */
    val index1 = index("record_idx1", lastName)
    /** Index over (gender) (database name record_idx2) */
    val index2 = index("record_idx2", gender)
    /** Index over (birthDate) (database name record_idx3) */
    val index3 = index("record_idx3", birthDate)
  }
